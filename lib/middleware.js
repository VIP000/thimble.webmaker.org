  ...
  return {
    ...
    checkForAuth: function(req, res, next) {
      if (!req.session.email || !req.session.username) {
        return next(new Error(req.gettext("please log in first")));
      }
      next();
    },
    ...
    requestForId: function(req, res, next) {
      if(!req.params.id) {
        return next(new Error(req.gettext("request did not point to a project")));
      }
      next();
    },
    ...
    checkForPublishData: function(req, res, next) {
      if(!req.body.html || req.body.html.trim() === "") {
        return next(new Error(req.gettext("no data to publish")));
      }
      next();
    },
        ...
        db.count({
          ...
          if (!edit && count > 1) {
            return next(req.gettext("You already have a page titled") + ' "' + req.pageTitle + '" ' +
                        req.gettext("change the text in the title"));
          }
    ...
    finalizeProject: function(nunjucksEnv, env) {
      ...
      return function(req, res, next) {
        ...
        req.body.finalizedHTML = sanitized.replace("</head", baseTag + "</head");
        next();
      };
    },
    ...
    publishData: function(options) {
      ...
      return function(req, res, next) {
        ...
        var location = req.publishLocation,
            ...
            s3PublishError = new Error(req.gettext("There was a problem publishing the page. Your page has been saved") +
